...
# -----------------------------------------------------------------------------
# Test 13: Infinite loop prevention (duplicate response page)
# -----------------------------------------------------------------------------
echo ""
echo "Test 13: Infinite loop prevention"
rm -rf ./paging
ERROR_OUTPUT=$("$CURL_PAGING" --cp "$BASE_URL/api/fault?mode=loop_trap&total_pages=3" 2>&1) || true

# Should fail with duplicate page error
if echo "$ERROR_OUTPUT" | grep -q "Duplicate response page"; then
    log_pass "Infinite loop prevention - duplicate page detected"
else
    log_fail "Infinite loop prevention - should detect duplicate page"
    echo "  Output: $ERROR_OUTPUT"
fi
