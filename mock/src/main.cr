...
get "/api/fault" do |env|
  ...
  when "loop_trap"
    # Cycle: page 1 → 2 → 1 → 2 ... (non-consecutive duplication)
    cycled_page = ((page - 1) % 2) + 1
    env.response.content_type = "application/json"
    {
      "data"       => build_items(cycled_page, 2),
      "pagination" => {
        "page"        => cycled_page,
        "total_pages" => total_pages,
      },
...
puts "  wrong_types, inconsistent_total, loop_trap (1→2→1 cycle), empty_data, slow"
puts ""
